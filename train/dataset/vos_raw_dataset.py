...
from train.dataset.vos_segment_loader import (
    JSONSegmentLoader,
...
class PNGRawDataset(VOSRawDataset):
    ...
    def get_video(self, idx):
        ...
        if self.single_object_mode:
            video_dir = os.path.dirname(video_name)  # 'video_0000_...'
            object_dir = os.path.basename(video_name)  # '000', '001'...
            cat_map = {'000': 'arm', '001': 'gripper', '002': 'robot'}  # category mapping
            category = cat_map[object_dir]
            video_frame_root = os.path.join(self.img_folder, video_dir)
        else:
        ...
        video = VOSVideo(video_name, idx, frames, category=category)
