...
class VideoEvaluator:
    def __init__(self, gt_root, pred_root, skip_first_and_last=True, category="robot") -> None:
        """
        ...
        # category to obj_dir mapping
        self.category2dir = {"arm": "000", "gripper": "001", "robot": "002"}
    ...
    def __call__(self, vid_name: str) -> Tuple[str, Dict[str, float], Dict[str, float]]:
        ...
        # scan the folder to find subfolders for evaluation and check if the folder structure is VRS
        to_evaluate, is_vrs_format = self.scan_vid_folder(vid_name)
        ...
        for all_frames, obj_id, gt_path, pred_path in to_evaluate:
            ...
            for frame in all_frames:
                gt_array, pred_array = self.get_gt_and_pred(
                    gt_path, pred_path, frame, is_vrs_format
                )
    ...
    def get_gt_and_pred(
        ...
        if is_vrs_format:
            assert len(np.unique(gt_array)) <= 2, (
                ...
                "VRS format assumes one object mask per png file."
            )
    ...
    def scan_vid_folder(self, vid_name) -> Tuple[List, bool]:
        """
        Scan the folder structure of the video and return a list of folders for evaluation.
        """
        ...
        if all(name.endswith(".png") for name in all_files_and_dirs):
            ...
            # VRS dataset structure, going one layer down into each subdirectory
            is_vrs_format = True

            if self.category in self.category2dir:
                target_dir = self.category2dir[self.category]
                if target_dir in all_files_and_dirs:
                    obj_dirs = [target_dir]
                else:
                    obj_dirs = []
            else:
                obj_dirs = all_files_and_dirs
        ...
        return to_evaluate, is_vrs_format
    ...
    *,
        ...
        if single_dataset:
            if verbose:
                results = tqdm.tqdm(
                    pool.imap(
                        VideoEvaluator(
                            gt_root, mask_root, skip_first_and_last=skip_first_and_last, category=category,
                        ),
                        videos,
                    ),
                    total=len(videos),
    ...
    for i, mask_root in enumerate(mask_roots):
        ...
        if verbose:
            print("\nSummary:")
            print(
                f"Global score J&F of {category}: {global_jf:.1f} J: {global_j:.1f} F: {global_f:.1f}"
            )
            print(f"Time taken: {time_taken:.2f}s")

        # print to file
        result_path = path.join(mask_root, f"results_{category}.csv")
